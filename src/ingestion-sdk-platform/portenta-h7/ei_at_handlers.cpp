...
static bool at_get_mgmt_url(void);
static bool at_set_mgmt_url(const char **argv, const int argc);
...
static bool at_run_impulse(void);
...
static bool at_clear_fs(void);
static bool at_reset(void);
static bool at_scan_wifi(void);
static bool at_get_wifi(void);
static bool at_set_wifi(const char **argv, const int argc);
/* Public functions -------------------------------------------------------- */
ATServer *ei_at_init(EiDevicePortenta* device)
{
    ATServer *at;
    dev = device;
...
static bool at_set_sample_settings(const char **argv, const int argc)
{
    ...
    if (argc >= 4) {
        ei_printf("sample settings hmac \n");
...
static bool at_get_snapshot(void)
{
    ...
    int r = dev->get_snapshot_list((const ei_device_snapshot_resolutions_t **)&list, &list_size, &color_depth);
